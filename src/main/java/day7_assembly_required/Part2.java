package day7_assembly_required;
...
public class Part2 {
    ...
    public void main() {
        ...
        for (String conn : inputs) {
            setWire(conn);
        }
        System.out.println(wires.get("a").getValue());
    }

    private void setWire(String conn) {
        Matcher m = p.matcher(conn);
        List<String> parts = new ArrayList<>();
        m.find();
        if (m.group(1) != null) parts.add(m.group(1).trim());
        if (m.group(2) != null) parts.add(m.group(2).trim());
        if (m.group(3) != null) parts.add(m.group(3).trim());
        if (m.group(4) != null) parts.add(m.group(4).trim());
        if (parts.size() == 4) {
            String id = parts.get(3);
            Wire wire = new Wire(id).setInput(parts.get(0), Operators.getOperator(parts.get(1)), parts.get(2));
            wires.put(id, wire);
        } else if (parts.size() == 3) {
            String id = parts.get(2);
            Wire wire = new Wire(id).setInput(Operators.NOT, parts.get(1));
            wires.put(id, wire);
        } else if (parts.size() == 2) {
            String id = parts.get(1);
            Wire wire = new Wire(id).setInput(parts.get(0));
            wires.put(id, wire);
        }
    }
    ...
    class Wire {
        ...
        @Getter private Integer mode;
        @Getter private String id;
        @Getter private String input1;
        @Getter private String input2;
        @Getter private Operators operator;

        Wire(String id) {
            this.id = id;
        }

        Wire setInput(String in1, Operators op, String in2) {
            mode = 3;
            input1 = in1;
            input2 = in2;
            operator = op;
        ...
        Wire setInput(Operators op, String in1) {
            mode = 2;
            input1 = in1;
            operator = op;
            return this;
        ...
        public Wire setInput(String in1) {
            mode = 1;
            input1 = in1;
            return this;
        }
        ...
        void setValue() {
            switch (mode) {
                case 1:
                    try {
                        value = Integer.valueOf(input1);
                    } catch (NumberFormatException e) {
                        value = wires.get(input1).getValue();
                    }
                    break;
                case 2:
                    value = ~wires.get(input1).getValue() & 0xFFFF;
                    break;
                case 3:
                    Integer val1, val2;
                    try { val1 = Integer.valueOf(input1); }
                    catch (NumberFormatException e) { val1 = wires.get(input1).getValue(); }
                    try { val2 = Integer.valueOf(input2); }
                    catch (NumberFormatException e) { val2 = wires.get(input2).getValue(); }
                    switch (operator) {
                        case AND:
                            value = val1 & val2;
                            break;
